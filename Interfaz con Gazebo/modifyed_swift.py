...
from swift import SwiftElement, Button
import swift as sw
import websockets
import asyncio
...
class Swift:
    ...
    def launch(self, realtime=False, headless=False, rate=60, browser=None, new_tab=True, **kwargs):
        """
        ...
        if not self.headless:
            ...
            self.socket, self.server, self.server_port, self.socket_port = start_servers(
                self.outq,
                self.inq,
...
def start_servers(outq, inq, stop_servers, open_tab=True, browser=None, dev=False):
    ...
    return socket, server, server_port, socket_port
